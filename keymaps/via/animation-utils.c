...
#define OLED_ROWS OLED_DISPLAY_HEIGHT / 8
...
static void oled_render_anim_frame(const char **fast_frames,const char **fast_rev_frames, const char **slow_frames, uint8_t frames_len, int start_row ) {

  static uint32_t anim_timer = 0;
  static uint8_t current_frame = 0;
  static int16_t frame_offset = 0; //ANIM_RENDER_WPM ? ANIM_WPM_WIDTH : 0;
  static int8_t step = 8;
  static bool is_bounce = false;
  ...
  if (timer_elapsed32(anim_timer) > ANIM_FRAME_TIME && speed > 0) {
    ...
    const char *frame = is_fast ? (is_bounce ? fast_rev_frames[current_frame] : fast_frames[current_frame]) : slow_frames[current_frame];
    const int8_t bg = ANIM_BG;

    uint8_t frame_start_offset = 0; //ANIM_RENDER_WPM ? ANIM_WPM_WIDTH : 0;
    current_frame = (current_frame + 1) % frames_len;

    for(int offset = 0; offset < OLED_MATRIX_SIZE / 2; offset++) {
      ...
      if(col < frame_start_offset) {
        ...
        oled_write_raw_byte((ANIM_INVERT ? ~bg : bg), offset + start_row * OLED_DISPLAY_WIDTH);
        ...
        oled_write_raw_byte(ANIM_INVERT ? ~col_data : col_data, offset + start_row * OLED_DISPLAY_WIDTH);
      }
    }
